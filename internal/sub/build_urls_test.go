...
func TestStripSubscriptionNamePreservesOtherQuery(t *testing.T) {
	got := stripSubscriptionName("https://example.com/sub/ABC?name=email&view=raw")
	if got != "https://example.com/sub/ABC?view=raw" {
		t.Fatalf("got %q", got)
	}
}

func TestBuildURLs_OmitsActualClientName(t *testing.T) {
	initSubDB(t)
	s := &SubService{}
	s.PrepareForRequest("sub.example.com")

	subURL, jsonURL, clashURL := s.BuildURLs("/sub/", "/json/", "/clash/", "ABC", "john@example.com")
	wantSub := "http://sub.example.com:2096/sub/ABC"
	wantJSON := "http://sub.example.com:2096/json/ABC"
	wantClash := "http://sub.example.com:2096/clash/ABC"
	if subURL != wantSub || jsonURL != wantJSON || clashURL != wantClash {
		t.Fatalf("plain subscription URLs = %q %q %q, want %q %q %q", subURL, jsonURL, clashURL, wantSub, wantJSON, wantClash)
	}
}
